...
import { Search, Tag, ArrowRight, ChevronLeft, ChevronRight, ChevronsUpDown } from 'lucide-react'
...
export default function BlogPage() {
  ...
  const { data, isLoading } = useBlogPosts({ search, page, limit: 12 })
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        ...
        <div className="relative mb-10 max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30"
          />
        </div>
              ...
              {posts.map((post, i) => (
                <motion.article key={post.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="group card overflow-hidden hover:shadow-lg transition-all duration-300">
                  <div className="relative h-48 bg-gradient-to-br from-primary-100 to-purple-100 dark:from-primary-900/30 dark:to-purple-900/30 overflow-hidden">
                    {post.coverImage && <Image src={buildImageUrl(post.coverImage)} alt={post.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover group-hover:scale-105 transition-transform duration-500" />}
                    {post.tags.length > 0 && (
