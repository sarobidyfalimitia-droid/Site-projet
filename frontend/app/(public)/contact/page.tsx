...
function validateForm(form: { name: string; email: string; subject: string; body: string }): FormErrors {
  const errors: FormErrors = {}
  if (!form.name.trim()) errors.name = 'Le nom est requis'
  else if (form.name.trim().length < 2) errors.name = 'Le nom doit contenir au moins 2 caractères'
  else if (form.name.trim().length > 100) errors.name = 'Le nom ne doit pas dépasser 100 caractères'

  if (!form.email.trim()) errors.email = 'L\'email est requis'
  else if (!EMAIL_REGEX.test(form.email.trim())) errors.email = 'Email invalide'

  if (!form.subject.trim()) errors.subject = 'Le sujet est requis'
  else if (form.subject.trim().length < 3) errors.subject = 'Le sujet doit contenir au moins 3 caractères'
  else if (form.subject.trim().length > 200) errors.subject = 'Le sujet ne doit pas dépasser 200 caractères'

  if (!form.body.trim()) errors.body = 'Le message est requis'
  else if (form.body.trim().length < 10) errors.body = 'Le message doit contenir au moins 10 caractères'
  else if (form.body.trim().length > 5000) errors.body = 'Le message ne doit pas dépasser 5000 caractères'
              ...
              <form onSubmit={handleSubmit} className="card p-8 space-y-5">
                  ...
                  {[
                    ...
                    <div key={key}>
                      ...
                      <input
                        ...
                        required={required}
                        className={`w-full px-3 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30 bg-gray-50 dark:bg-gray-800 ${
                      ...
                      />
                      {errors[key as keyof FormErrors] && (
                        <p className="text-red-500 text-xs mt-1">{errors[key as keyof FormErrors]}</p>
                      )}
                    </div>
                  ))}
                </div>
                <div>
                  ...
                  <input
                    ...
                    required
                    className={`w-full px-3 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30 bg-gray-50 dark:bg-gray-800 ${
                      errors.subject ? 'border-red-400 dark:border-red-500' : 'border-gray-200 dark:border-gray-700'
                    }`}
                  />
                ...
                <div>
                  ...
                  <textarea
                    ...
                    required
                    className={`w-full px-3 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30 bg-gray-50 dark:bg-gray-800 resize-none ${
                      errors.body ? 'border-red-400 dark:border-red-500' : 'border-gray-200 dark:border-gray-700'
                    }`}
                  />
